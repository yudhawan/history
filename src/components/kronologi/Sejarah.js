...
function Sejarah({show, handleSejarah}) {
  return (
    <div className={`${show?'block fixed':'hidden'} top-0 self-center flex justify-center bg-transparent backdrop-blur-sm items-center w-full h-full lg:bg-opacity-70 bg-opacity-80 bg-black`}>
        <div className='bg-gray-400 rounded-full z-10 w-8 h-8 justify-center items-center flex hover:animate-spin absolute cursor-pointer -mt-[79vh] -mr-[88vw] lg:-mr-[90vw] text-rose-700' onClick={handleSejarah}><FontAwesomeIcon icon={faXmark} size={'lg'} /></div>
