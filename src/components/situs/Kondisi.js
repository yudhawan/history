...
function Kondisi({selected, handleselected}) {
    ...
    <div className='flex flex-wrap  lg:flex-row justify-center items-center'>
      {
        ...
        knds.map((item,index)=>{
          return(
            <div className='flex-1 lg:p-2 cursor-pointer mx-1 my-1 select-none' key={index} onClick={()=>handleselected(item)} >
      ...
      <div className={`${selected?'block fixed':'hidden'} top-20 self-center flex justify-center bg-transparent backdrop-blur-sm  w-[90vw] h-full lg:bg-opacity-70 bg-opacity-60 bg-black`}>
                            ...
                            <>
                                ...
                                {val.images&&val.images.map((img,index)=>(
                                    <div key={index} className='w-72 h-64 self-center mx-1 my-1 lg:mx-2 lg:my-2 rounded-md relative'>
                                        ...
                                        <div className='text-xs text-black absolute bottom-0 right-0 text center rounded-md px-[1px] bg-white '>Sumber: {img.sumber}</div>
                                    </div>
                                ))}
                                </div>
