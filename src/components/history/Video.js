...
function Video({show,handleVideo}) {
  return (
    <div className={`${show?'block fixed':'hidden'} top-0 self-center flex justify-center bg-transparent backdrop-blur-sm items-center w-full h-full lg:bg-opacity-70 bg-opacity-80 bg-black`}>
      <div className='bg-white rounded-md w-[90vw] h-auto sm:h-[70vh] lg:h-[96vh] '>
        ...
        <div className='text-justify lg:font-Roboto overflow-auto p-4 space-y-2 w-full h-[30vh] sm:h-[60vh] lg:h-[90vh]'>
          <iframe src='https://www.youtube.com/embed/USGIbaNKqQo' className='w-full h-full' frameBorder="0" allow="accelerometer; clipboard-write;encrypted-media; gyroscope; picture-in-picture"  allowFullScreen />
        </div>
      </div>
    </div>
  )
